...
NSString *const kPXPlayerFinishedPlayNotificationKey  = @"com.hotoday.kHTPlayerFinishedPlayNotificationKey";
NSString *const kPXPlayerCloseVideoNotificationKey    = @"com.hotoday.kHTPlayerCloseVideoNotificationKey";
NSString *const kPXPlayerCloseDetailVideoNotificationKey    = @"com.hotoday.kHTPlayerCloseDetailVideoNotificationKey";
NSString *const kPXPlayerFullScreenBtnNotificationKey = @"com.hotoday.kHTPlayerFullScreenBtnNotificationKey";
NSString *const kPXPlayerPopDetailNotificationKey = @"com.hotoday.kHTPlayerPopDetailNotificationKey";
...
- (instancetype)initWithFrame:(CGRect)frame videoURLStr:(NSString *)videoURLStr{
    ...
    if (self) {
        ...
        self.screenType = UIPXPlayerSizeRecoveryScreenType;
    }
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        if(status==AVPlayerStatusReadyToPlay){
            ...
            [self initTimer];
            if (self.status)self.status(UIPXPlayerStatusReadyToPlayTyep);
        ...
        if (self.alpha == 0.00) {
            if (self.status)self.status(UIPXPlayeStatusrLoadedTimeRangesType);
...
-(void)fullScreenAction:(UIButton *)sender{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kPXPlayerFullScreenBtnNotificationKey object:sender];
}

//关闭播放视频通知
-(void)colseTheVideo:(UIButton *)sender{
    [self.player pause];
    [[NSNotificationCenter defaultCenter] postNotificationName:kPXPlayerCloseVideoNotificationKey object:sender];
}

//关闭详情视频
- (void)colseDetailVideo:(UIButton *)sender{
    [[NSNotificationCenter defaultCenter] postNotificationName:kPXPlayerPopDetailNotificationKey object:self];
    [[NSNotificationCenter defaultCenter] postNotificationName:kPXPlayerCloseDetailVideoNotificationKey object:sender];
}
...
- (void)setVideoURLStr:(NSString *)videoURLStr
{
    ...
    if (self.status)self.status(UIPXPlayerStatusLoadingType);
}
- (void)moviePlayDidEnd:(NSNotification *)notification {
    ...
    [self.player seekToTime:kCMTimeZero completionHandler:^(BOOL finished) {
        ...
        //播放完成后的通知
        [[NSNotificationCenter defaultCenter] postNotificationName:kPXPlayerFinishedPlayNotificationKey object:nil];
    }];
}
//#pragma mark - 播放进度
...
-(void)toFullScreenWithInterfaceOrientation:(UIInterfaceOrientation )interfaceOrientation{
    ...
    self.screenType = UIPXPlayerSizeFullScreenType;
...
-(void)toDetailScreen:(UIView *)view
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.screenType = UIPXPlayerSizeDetailScreenType;
...
-(void)reductionWithInterfaceOrientation:(UIView *)view{
    ...
    if (self.screenType == UIPXPlayerSizeSmallScreenType) {
...
- (void)reduction:(UIView *)view
{
    ...
    [view bringSubviewToFront:self];
    self.backView.alpha= 0;
    float duration = self.screenType == UIPXPlayerSizeFullScreenType?0.5f:0.0f;
...
-(void)toSmallScreen{
    ...
    self.screenType = UIPXPlayerSizeSmallScreenType;
...
- (void)setScreenType:(UIPXPlayerSizeType)screenType{
    _screenType = screenType;
    if (screenType == UIPXPlayerSizeSmallScreenType || screenType == UIPXPlayerSizeDetailScreenType) {
        _closeBtn.hidden = NO;
        ...
        if (screenType == UIPXPlayerSizeSmallScreenType) {
