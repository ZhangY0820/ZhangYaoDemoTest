//
//  PXForgetPasswordViewController.swift
//  FinancialAdviserApp
//
...
class PXForgetPasswordViewController: PXBaseViewController, UITextFieldDelegate {
    ...
    func creatUI()  {
        ...
        let labelTextArr = ["手机号", "验证码"]
        let textFieldPlaceholderArr = ["请填写手机号码", "请填写验证码"]
        _passWordView.sendCodeBtn?.isHidden = false
        _passWordView.sendCodeBtn?.addTarget(self, action: #selector(sendCode), for: .touchUpInside)
        for i in 0..<2 {
    ...
    func threeButtonCreat()  {
        ...
        _nextBtn.setTitle("下一步", for: .normal)
